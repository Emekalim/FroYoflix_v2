<script context='module'>
  ...
  const key = writable({})
  const search = writable(cache.getEntry(caches.HISTORY, 'lastSchedule') || { scheduleList: true, format: ['Anime'], format_not: [], genre: [], genre_not: [], tag: [], tag_not: [], status: [], status_not: [] })
  ...
  async function fetchAllScheduleEntries (variables) {
    ...
    // Fetch anime TV schedule from AniList
    ...
    // Query AniList for anime schedule
    ...
    if (variables.hideSubs) {
      ...
      results.data.Page.media = results.data.Page.media.filter((media, index, self) => {
        ...
        if (!cachedItem) return false // Skip anime without dub info
        ...
        return (!(cachedItem?.media?.media?.airingSchedule?.nodes[0]?.episode > media.episodes) || !media.episodes) && (!predict || !((numberOfEpisodes > 4) && !cachedItem.unaired)) && cachedItem?.media?.media?.airingSchedule?.nodes[0]?.airingAt && self.findIndex(m => m.id === media.id) === index
          ...
          const airingLists = animeSchedule.dubAiringLists.value
      ...
      // Simple sorting for anime subs
      results.data.Page.media = results.data.Page.media.filter((media, index, self) => {
        return nextAiring(media?.airingSchedule?.nodes)?.airingAt && self.findIndex(m => m?.id === media?.id) === index
      }).sort((a, b) => {
        const aAiring = nextAiring(a.airingSchedule?.nodes)?.airingAt
        const bAiring = nextAiring(b.airingSchedule?.nodes)?.airingAt
