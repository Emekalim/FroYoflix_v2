...
<script>
  ...
  function ratioType(ratio, progress) {
    if (
      (Math.round(ratio * 100) / 100 <= 0 || progress < 1) &&
      !data.incomplete
    )
      return "Leeching";
  ...
  $: episode =
    resolved?.files?.length <= 1
      ? !resolved?.files?.length
        ? resolved?.episode
        : Number.isFinite(Number(resolved?.files?.[0]?.episode))
          ? Number(resolved?.files?.[0]?.episode)
          : resolved?.episode
      : null;
  $: episodeRange =
    resolved?.files?.length <= 1
      ? !resolved?.files?.length
        ? resolved?.episodeRange &&
          `${resolved.episodeRange.first} ~ ${resolved.episodeRange.last}`
        : (resolved?.files?.[0]?.episodeRange &&
            `${resolved?.files?.[0].episodeRange.first} ~ ${resolved?.files?.[0].episodeRange.last}`) ||
          (resolved?.episodeRange &&
            `${resolved.episodeRange.first} ~ ${resolved.episodeRange.last}`)
      : null;
  $: search = $mediaCache[resolvedId]
    ? { media: $mediaCache[resolvedId], episode, episodeRange }
    : null;
  ...
  const { reactive: _reactive, init: _init } = createListener([
    `react-${infoHash}`,
  ]);
  onMount(() => {
    init(true, true);
...
<div
  role="button"
  tabindex="0"
  class="details border-top py-20 text-wrap text-break-word d-flex {$_reactive &&
  !current
    ? ``
    : `not-reactive`}"
  class:bg-error={completed && data.incomplete}
  class:current
  class:option={!current}
  class:pointer={!current}
  class:not-allowed={current}
...
>
  <div class="d-flex flex-row w-full load-in mw-0">
    <div class="p-5 ml-20 name mw-150 flex-1 w-auto d-flex flex-column">
      {#if resolvedId && $mediaCache[resolvedId]}
        <div
          class="font-weight-bold line-2 overflow-hidden d-flex align-items-center justify-content-center"
          title={`${anilistClient.title($mediaCache[resolvedId])}${episode || episode === 0 ? ` Episode ${episode}` : ``}`}
        >
          {#if episodeRange || episode || episode === 0}
            <span
              class="mr-2 d-inline-flex align-items-center justify-content-center align-middle bg-primary-dim px-3 rounded mb-5"
              title={`Episode ${episodeRange || episode}`}
              ><Clapperboard size="2rem" class="mr-5" /><span class="mt-2 mr-2"
                >{episodeRange || episode}</span
              ></span
            >
          {:else if $mediaCache[resolvedId]?.format === "MOVIE"}
            <span
              class="mr-2 d-inline-flex align-items-center align-middle bg-primary-dim px-3 py-3 rounded mb-5"
              title="Movie"><Projector size="2rem" /></span
            >
          {:else}
            <span
              class="mr-2 d-inline-flex align-items-center align-middle bg-primary-dim px-3 py-3 rounded mb-5"
              title="Batch"><Database size="2rem" /></span
            >
          {/if}
      ...
      {/if}
      <div
        class="text-muted line-2 overflow-hidden"
        title={data.name && MediaResolver.cleanFileName(data.name)}
      >
        {(data.name && MediaResolver.cleanFileName(data.name)) || "—"}
      </div>
    </div>
    <div class="p-5 w-150 d-none d-md-block">{fastPrettyBytes(data.size)}</div>
    <div class="p-5 w-150">
      {data.size && !data.missing_pieces
        ? `${(data.progress * 100 || 0).toFixed(1)}%`
        : data.missing_pieces
          ? "—"
          : "0.0%"}
    </div>
    ...
    <div class="p-5 w-150 d-none d-md-block">
      {(!completed &&
        data.name &&
        (data.progress
          ? (Math.ceil((data.ratio || 0) * 100) / 100)?.toFixed(2)
          : "0.00")) ||
        (data.incomplete && !data.missing_pieces ? "0.01" : "—")}<span
        class="text-muted text-nowrap"
        class:d-none={(completed &&
          (!data.incomplete || data.missing_pieces)) ||
          !data.name}>{` (${ratioType(data.ratio || 0, data.progress)})`}</span
      >
    </div>
    <div class="p-5 w-150">
      {completed ? "—" : `${fastPrettyBytes(data.downloadSpeed || 0)}/s`}
    </div>
    <div class="p-5 w-150 d-none d-md-block">
      {completed ? "—" : `${fastPrettyBytes(data.uploadSpeed || 0)}/s`}
    </div>
    <div class="p-5 w-150">
      {completed ? "—" : data.numSeeders || 0}<span
        class="text-muted text-nowrap"
        class:d-none={completed}>{` (${data.totalSeeders || 0})`}</span
      >
    </div>
    <div class="p-5 w-150 d-none d-md-block">
      {completed ? "—" : data.numLeechers || 0}<span
        class="text-muted text-nowrap"
        class:d-none={completed}>{` (${data.numPeers || 0})`}</span
      >
    </div>
    <div class="p-5 w-115 d-none d-md-block">
      {data.eta > 0 && data.progress < 1
        ? eta(new Date(Date.now() + data.eta))
        : "∞"}
    </div>
  </div>
  <div
    class="dropdown react-{infoHash} with-arrow right-0 mr-5 mr-md-20 w-40 h-auto"
  ...
  >
    ...
    >
    <div
      class="dropdown-menu dropdown-menu-right pt-5 pb-5 ml-10 text-capitalize w-auto hm-400 text-nowrap"
    >
      <div
        role="button"
        class="pointer d-none align-items-center justify-content-center font-size-16 rounded option details py-5 px-10"
        class:d-flex={!current}
      ...
      <div
        role="button"
        class="pointer d-flex align-items-center justify-content-center font-size-16 rounded option details py-5 px-10"
      ...
      <div
        role="button"
        class="pointer d-none align-items-center justify-content-center font-size-16 rounded option details py-5 px-10"
        class:d-flex={resolvedId && $mediaCache[resolvedId]}
      ...
      <div
        role="button"
        class="pointer d-none align-items-center justify-content-center font-size-16 rounded option details py-5 px-10"
        class:d-flex={data.magnetURI}
        aria-label="Copy Magnet Link"
        title="Copy Magnet Link"
        use:click={() => {
          copyToClipboard(data.magnetURI, "magnet URL");
          toggleDropdown();
        }}
      >
        Copy Magnet
...
<style>
  ...
  .current {
    border: 0.1rem solid var(--quaternary-color) !important;
  }
  .details {
