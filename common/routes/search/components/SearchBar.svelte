...
<script>
  import { traceAnime, genreIcons, genreList, tmdbGenreList, tagList } from '@/modules/anime/anime.js'
  ...
  // Function to get active genres based on format selection
  function getActiveGenres() {
    const hasAnime = !search.format || search.format.length === 0 || search.format.includes('Anime')
    const hasTMDB = search.format && (search.format.includes('TV Shows') || search.format.includes('Movies'))
    
    // If only TMDB selected, use TMDB genres; if mixed or anime only, use anime genres
    return hasTMDB && !hasAnime ? tmdbGenreList : genreList
  }

  // Function to get active tags based on format selection
  function getActiveTags() {
    const hasAnime = !search.format || search.format.length === 0 || search.format.includes('Anime')
    return hasAnime ? tagList : []
  }
  ...
  function getTags() {
    ...
    const headers = {}
    if (activeGenres.length > 0) headers[activeGenres[0]] = 'Genres'
    if (activeTags.length > 0) headers[activeTags[0]] = 'Tags'
  ...
  $: {
    const activeGenres = getActiveGenres()
    const activeTags = getActiveTags()
    search.genre = searchTags.tags.filter(val => activeGenres.includes(val))
    search.tag = searchTags.tags.filter(val => activeTags.includes(val))
    search.genre_not = searchTags.tags_not.filter(val => activeGenres.includes(val))
    search.tag_not = searchTags.tags_not.filter(val => activeTags.includes(val))
  }
  ...
  <div class='row'>
    ...
    <div class='col-lg col-4 p-10 z-4 d-flex {advancedSearch} flex-column justify-content-end'>
      ...
      <div class='input-group' title={(!Helper.isAniAuth() && Helper.isUserSort(search)) ? 'Cannot use with sort: ' + sortOptions[search.sort] : ''}>
        <CustomDropdown id={`tags-input`} bind:form headers={searchTags.headers} options={[...toArray(getActiveGenres()), ...toArray(getActiveTags())]} bind:value={searchTags.tags} bind:altValue={searchTags.tags_not} constrainAlt={false} disabled={search.disableSearch || (!Helper.isAniAuth() && Helper.isUserSort(search))}/>
      </div>
    </div>
    ...
    <div class='col p-10 z-3 d-flex {advancedSearch} flex-column justify-content-end'>
      ...
      <div class='input-group'>
        <CustomDropdown id={`format-input`} bind:form options={{ Anime: 'Anime', 'TV Shows': 'TV Shows', Movies: 'Movies' }} bind:value={search.format} bind:altValue={search.format_not} bind:disabled={search.disableSearch}/>
      </div>
    </div>
