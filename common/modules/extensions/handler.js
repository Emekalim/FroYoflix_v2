...
export async function getResultsFromExtensions({ media, episode, season, batch, movie, resolution }) {
  ...
  // Build unified IDs object for all media types
  const ids = {}
    ...
    if (!source?.nsfw || settings.value.adult !== 'none') {
      const promise = (async () => {
        try {
          ...
          debug(`Extension ${key} - results: ${results?.length}, errors: ${errors?.length}`)
          // Only throw error if we have errors AND no results. If we got results, ignore the errors
          if (errors?.length && !results?.length && !JSON.stringify(errors)?.match(/no anidb[ae]id provided/i)) throw new Error(errors?.map(error => (error?.message || JSON.stringify(error)).replace(/\\n/g, ' ').replace(/"/g, '')).join('\n') || 'Unknown error')
          else if (errors?.length && !results?.length) throw new Error('Source ' + source.id + ' found no results.')
