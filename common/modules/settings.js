...
function setDefaults() {
  scopedDefaults = {
    homeSections: [...(storedSettings.rssFeedsNew || defaults.rssFeedsNew).map(([title]) => [title, ['N/A'], ['N/A']]), ...(storedSettings.customSections || defaults.customSections).map(([title]) => [title, 'TRENDING_DESC', ['Anime', 'MOVIE', 'TV']]), ['Subbed Releases', 'N/A', ['Anime', 'MOVIE', 'TV']], ['Dubbed Releases', 'N/A', ['Anime', 'MOVIE', 'TV']], ['Continue Watching', 'UPDATED_TIME_DESC', []], ['Sequels You Missed', 'POPULARITY_DESC', []], ['Planning List', 'POPULARITY_DESC', []], ['Popular This Season', 'N/A', ['Anime', 'MOVIE', 'TV']], ['Trending Now', 'N/A', ['Anime', 'MOVIE', 'TV']], ['All Time Popular', 'N/A', ['Anime', 'MOVIE', 'TV']]]
  }
}
    ...
    if (clipboardData.items[0].type === 'text/plain' && clipboardData.items[0].kind === 'string') {
      clipboardData.items[0].getAsString(text => {
        if (text.includes('access_token=')) { // is an AniList token
          ...
          let protocol = text
          if (anilistMatch) {
            protocol = `froyo://anime/${anilistMatch[1]}`
          } else if (malMatch) {
            protocol = `froyo://malanime/${malMatch[1]}`
          }
