...
export function getUpdaterPrimaryAction(state) {
  switch (state.phase) {
    ...
    case UPDATE_PHASES.AVAILABLE:
      return { label: 'Download Update', action: 'download', disabled: !state.canDownload }
    ...
    case UPDATE_PHASES.DEFERRED:
      ...
      if (state.canDownload) return { label: 'Download Update', action: 'download', disabled: false }
      return { label: 'Check for Updates', action: 'check', disabled: false }
    default:
      return { label: 'Check for Updates', action: 'check', disabled: false }
  }
...
export function getUpdaterStatusMessage(state) {
  switch (state.phase) {
    ...
    case UPDATE_PHASES.AVAILABLE:
      return state.targetVersion ? `v${state.targetVersion} is available to download.` : 'A new update is available.'
    case UPDATE_PHASES.DOWNLOADING:
    ...
    case UPDATE_PHASES.DEFERRED:
      return state.targetVersion ? `v${state.targetVersion} has been deferred for this session.` : 'Update reminder deferred for this session.'
    default:
