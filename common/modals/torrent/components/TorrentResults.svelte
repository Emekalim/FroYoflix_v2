<script context="module">
  ...
  function isMovie(media) {
    ...
    if (
      ...
      return true; // TODO: revisit this, it causes false positives since the term "movie" is used randomly on shorts that are clearly not movie length.
   ...
   */
  async function getRequestedAudio(
    search,
    result,
    audioLang,
    exactMatch = true,
    ...
    const terms = [
      ...new Map(
        (await sanitiseTerms(search, result))?.map((term) => [
          term.term.text,
          term.term,
        ]),
      ).values(),
    ];
    const checkTerm = (term, keyword) =>
      (Array.isArray(term.text) ? term.text : [term.text]).some((text) =>
  ...
  async function getBest(search, results, audioLang, torrentProvider = []) {
    ...
    if (audioLang !== "jpn") {
      const checks = await Promise.all(
        results.map(async (result) => ({
          result,
          exactBest:
            (await getRequestedAudio(search, result.parseObject, audioLang)) &&
            result.seeders > 9,
          exactAlt:
            (await getRequestedAudio(
              search,
              result.parseObject,
              audioLang,
              false,
      ...
      candidates.push(
        ...checks
          .filter((check) => check.exactBest)
          .map((check) => check.result),
        ...checks
          .filter((check) => check.exactAlt)
          .map((check) => check.result),
        ...checks
          .filter((check) => check.dualBest)
          .map((check) => check.result),
        ...checks.filter((check) => check.dualAlt).map((check) => check.result),
      );
    }
    candidates.push(
      ...results.filter(
        (result) =>
          (result.type === "best" || result.type === "alt") &&
          result.seeders > 9,
      ),
    );
    ...
    if (torrentProvider?.length) {
      const filteredByProvider = toConsider.filter(
        (result) =>
          result.parseObject?.release_group &&
          torrentProvider.some((provider) =>
            result.parseObject.release_group
              .toLowerCase()
  ...
  function filterResults(results, searchText) {
    ...
    return (
      results.filter(({ title }) =>
        matchPhrase(searchText, title, 0.4, false, true),
      ) || []
    );
  }
  ...
  function sortResults(results, sort) {
    ...
    const deduped = Array.from(dedupe(results)).map((result) => {
      if (
        !(
          result.parseObject?.release_group &&
          result.parseObject.release_group.length < 20
        )
      )
        result.parseObject = {
          ...result.parseObject,
          release_group: "No Group",
        };
      return result;
    });
    return {
...
<script>
  ...
  const updateEpisode = debounce((value) => {
    if (search.episode !== value) search.episode = value;
  }, 500);
  ...
  function dubFinished() {
    const airingMedia = animeSchedule.dubAiring.value?.find(
      (entry) => entry.media?.media?.id === search.media.id,
    );
    return (
      !airingMedia ||
      (airingMedia.episodeNumber === search.media.episodes &&
        new Date().getTime() >= new Date(airingMedia.episodeDate).getTime()) ||
      (search.media.mediaListEntry?.progress ?? 0) > airingMedia.episodeNumber
    );
  }
  ...
  async function addCachedHashes(cachedHashes) {
    ...
    for (const cached of cachedHashes) {
      const torrent = torrents.find(
        (torrent) => torrent.infoHash === cached.hash,
      );
      if (!torrent) continue;
      const title = MediaResolver.cleanFileName(torrent.name);
      let searchEpisode = search?.episode;
      let isLocked =
        cached.locked ??
        (cached.files?.length === 1 &&
          (cached.files[0].locked || cached.files[0].parseObject?.locked)) ??
        false;
      if (!isLocked && Array.isArray(cached.files) && searchEpisode != null) {
        const normalizedEpisode = Number.isFinite(Number(searchEpisode))
          ? Number(searchEpisode)
          : searchEpisode;
        const matchingFile = cached.files.find(
          (file) => file.episode === normalizedEpisode,
        );
    ...
    if (cachedTorrents.length)
      results.update((result) => ({
        ...result,
        torrents: [...(result?.torrents ?? []), ...cachedTorrents],
      }));
  }
  ...
  async function queryExtensions(request, resolution) {
    $results = {};
    const cachedHashes = [];
    for (const resolvedHash of getHash(
      search?.media?.id,
      { episode: search?.episode, client: true, batchGuess: true },
      false,
      true,
      true,
    ) ?? []) {
      if (resolvedHash) {
        const cachedFile = getId(
          resolvedHash,
    ...
    let promises;
    try {
      promises = await getResultsFromExtensions({
        ...request,
      ...
      if (
        search != null &&
        search.media?.id === request?.media?.id &&
        search.episode === request?.episode
      ) {
    ...
    debug(
      `Query promises from extensions have been accepted for ${search?.media?.id}`,
    );
    return promises;
  }
  ...
  async function getErrors(request, promises) {
    ...
    await (async () => {
      (
        await Promise.all(
          Array.from(queries, ([_, extension]) => extension.promise),
        )
      ).forEach((result) => {
        if (result.errors && result.errors.length > 0)
          result.errors.forEach((error) => uniqueErrors.add(error.message));
      });
    })();
    if (
      search == null ||
    ...
    results.update((r) => ({ ...r, resolved: true }));
    debug(
      `All query promises have successfully been resolved for ${search?.media?.id}:E${search?.episode}`,
      JSON.stringify(Array.from(uniqueErrors)),
    );
    if (
      $status !== "offline" &&
      JSON.stringify(Array.from(uniqueErrors)).match(/found no results/i) &&
      (search?.media?.status !== "FINISHED" || !search?.media?.episodes) &&
      getMediaMaxEp(search?.media, true) < search?.episode
    )
      return {
        errors: [
          {
            message: `${anilistClient.title(search.media)} ${search.media?.format !== "MOVIE" || getMediaMaxEp(search?.media, false) > 1 ? `Episode ${search.media.nextAiringEpisode?.episode || search.episode}` : ``} hasn't released yet! ${search?.media?.nextAiringEpisode?.timeUntilAiring ? `\n${search.media?.format !== "MOVIE" || getMediaMaxEp(search?.media, false) > 1 ? `This episode` : `This movie`} will be released on ${new Date(Date.now() + search.media.nextAiringEpisode?.timeUntilAiring * 1000).toDateString()}` : ""}`,
          },
        ],
      };
  ...
  async function handleScrape() {
    ...
    toast.promise(
      (async () => {
        try {
          scraping = true;
          const beforeScrape = $results.torrents.map((torrent) => ({
            hash: torrent.hash,
          ...
          const changedCount = updatedResults.filter((torrent, index) => {
            ...
            return (
              before &&
              (torrent.seeders !== before.seeders ||
                torrent.leechers !== before.leechers)
            );
          }).length;
      ...
      })(),
      {
        ...
        success: (data) => {
          if (data.changed === 0)
            return `Peer counts are up to date! All ${data.total} torrent${data.total === 1 ? "" : "s"} checked, no changes detected.`;
          if (data.changed === data.total)
            return `Successfully refreshed peer counts for ${data.total} torrent${data.total === 1 ? "" : "s"}!`;
          return `Successfully refresh peer counts and found differences for ${data.changed} of ${data.total} torrent${data.total === 1 ? "" : "s"}`;
        },
        error: (error) =>
          `Failed to scrape peer data: ${error?.message || "Please try again later."}`,
      },
    );
  }
  ...
  const lastMagnet =
    cache.getEntry(caches.HISTORY, "lastMagnet")?.[`${search?.media?.id}`]?.[
      `${search?.episode}`
    ] ||
    cache.getEntry(caches.HISTORY, "lastMagnet")?.[`${search?.media?.id}`]
      ?.batch;
  ...
  function play(result) {
    ...
    cache.setEntry(caches.HISTORY, "lastMagnet", {
      ...existingMagnets,
      [search?.media?.id]:
        !result.parseObject?.episode_number ||
        Array.isArray(result.parseObject.episode_number)
          ? { [`batch`]: result }
          : {
              ...(existingMagnets[search?.media?.id] || {}),
              [`${search.episode}`]: result,
            },
    });
    add(
      result.link,
...
<div
  class="controls w-full bg-very-dark position-sticky top-0 z-10 pt-20 pb-10 px-30 mb-10"
>
  <div class="d-flex">
    <h3 class="mb-0 font-weight-bold text-white title mr-5 font-scale-40">
      {anilistClient.title(search?.media)}
    </h3>
    <button
      type="button"
      class="btn btn-square bg-dark-very-light ml-auto d-flex align-items-center justify-content-center rounded-2 flex-shrink-0"
      use:click={close}><X size="1.7rem" strokeWidth="3" /></button
    >
    <div class="position-absolute top-0 left-0 w-full h-full z--1">
      <div class="position-absolute w-full h-full overflow-hidden">
        <SmartImage
          ...
          images={[
            ...
            () =>
              getEpisodeMetadataForMedia(search.media).then(
      ...
      <div
        class="position-absolute top-0 left-0 w-full h-full"
        style="background: var(--torrent-banner-gradient)"
      />
    </div>
  </div>
  <div class="input-group mt-20 h-40 long-input z-11">
    ...
    />
    <div
      class="dropdown primary dropleft with-arrow position-absolute z-20 h-full right-0"
      use:click={() => {
    ...
    >
      <button
        type="button"
        class="options h-full bg-transparent shadow-none border-0 pointer p-0 pr-10 muted d-flex align-items-center"
        title="More Options"><EllipsisVertical size="2rem" /></button
      >
      <div
        class="position-absolute visibility top-0 text-capitalize hm-40 text-nowrap bg-dark-very-light dmr-arrow d-flex flex-column"
        class:hidden={!$showOptions}
      >
        <div
          class="dropdown dropleft with-arrow z-20 pointer p-5 rounded-1 option"
        ...
        >
          <div class="d-flex align-items-center pr-5">
            <ChevronLeft
              ...
              strokeWidth={2.5}
              class="flex-shrink-0"
            /><span class="ml-10 flex-grow-1 text-center">Preferred Audio</span>
          </div>
          <div
            class="dropdown-menu dropdown-menu-right text-capitalize text-nowrap"
          >
            <div class="custom-radio overflow-y-auto overflow-x-hidden hm-400">
              {#each languages as language}
                <input
                  ...
                  type="radio"
                  id="audio-{language.value}-radio"
                  value={language.value}
                  checked={language.value === $settings.audioLanguage}
                />
                <label
                  for="audio-{language.value}-radio"
                  use:click={() => {
                    $settings.audioLanguage = language.value;
                  }}
                  class="pb-5"
                >
                  {language.label}
        ...
        <div
          class="dropdown dropleft with-arrow z-20 pointer p-5 rounded-1 option"
          aria-label="Auto-Scrape Results"
          title="Auto-Scrape Results"
          use:click={toggleDropdown}
        >
          <div class="d-flex align-items-center pr-5">
            <ChevronLeft
              ...
              strokeWidth={2.5}
              class="flex-shrink-0"
            /><span class="ml-10 flex-grow-1 text-center"
              >Auto-Scrape Results</span
            >
          </div>
          <div
            class="dropdown-menu dropdown-menu-right text-capitalize text-nowrap mw-80"
          >
            <div class="custom-radio overflow-y-auto overflow-x-hidden">
              <input
                name="scrape-radio-set"
                type="radio"
                id="scrape-on-radio"
                value="on"
                checked={$settings.torrentAutoScrape}
              />
              <label
                for="scrape-on-radio"
                use:click={() => ($settings.torrentAutoScrape = true)}
                class="pb-5">On</label
              >
              <input
                name="scrape-radio-set"
              ...
              />
              <label
                for="scrape-off-radio"
                use:click={() => ($settings.torrentAutoScrape = false)}
                class="pb-5">Off</label
              >
            </div>
          </div>
        </div>
  ...
  <div class="row mt-10">
    ...
    >
      <div class="d-flex align-items-center mr-5" title="Toggle Autoplay">
        <Timer
          size="2.75rem"
          class="position-absolute z-10 text-dark-light pl-10 pointer-events-none"
        />
        <button
          type="button"
          class="form-control w-full bg-dark-very-light pointer control text-nowrap {!$settings.rssAutoplay
            ? `pl-15`
            : `px-25`}"
          use:click={() => autoPlayToggle()}
        >
          <span
            class:ml-20={!$settings.rssAutoplay}
            class:ml-10={$settings.rssAutoplay}
          >
            {#if $settings.rssAutoplay}
              Autoplay [{countdown}]
            {:else}
      ...
      <div
        ...
        style="width: calc(5.2rem + {(String(episodeSearch).length <= 10
          ? String(episodeSearch).length
          : 10) * 1}rem) !important"
        title="Episode"
      >
        <Clapperboard
        ...
        <input
          ...
          pattern="[0-9]*"
          max={maxEpisode}
          class="form-control bg-dark-very-light pl-40 control"
          placeholder="5"
          step="1"
          value={episodeSearch}
          on:input={episodeInput}
          disabled={(!search.episode && search.episode !== 0) || movie}
        />
      </div>
    </div>
    <div
      class="col-12 col-sm-6 d-flex align-items-center mt-5 justify-content-center mt-sm-0 justify-content-sm-end"
    >
      <div
        class="d-flex align-items-center mr-5"
        data-toggle="tooltip"
        data-placement="top"
        data-title="Scrape Peer Data"
      >
        <button
          type="button"
          class="btn btn-square bg-dark-very-light ml-auto d-flex align-items-center justify-content-center rounded-2 flex-shrink-0"
          use:click={handleScrape}
          disabled={!$results?.resolved ||
            !$results?.torrents?.length ||
            scraping}
          ><RefreshCw
            size="1.8rem"
            class={scraping ? "spinning" : ""}
          /></button
        >
      </div>
      <div class="d-flex align-items-center pr-5" title="Sorting Preference">
        ...
        />
        <select
          class="form-control w-full bg-dark-very-light pl-40 control"
          bind:value={$settings.torrentSort}
        >
      ...
      <div class="d-flex align-items-center" title="Video Quality">
        ...
        <select
          class="form-control w-full bg-dark-very-light pl-40 control"
          bind:value={$settings.rssQuality}
        >
...
<div class="mt-10 mb-sm-10 px-30">
  ...
  {:else}
    ...
    {:else if $results?.torrents?.length}
      {#if best}<TorrentCard
          type="best"
          countdown={$settings.rssAutoplay && $results?.resolved
            ? countdown
            : -1}
          result={best}
          {play}
    ...
    {#if lookupHidden?.length && $results?.resolved && filterResults(lookupHidden, searchText)?.length}
      <button
        type="button"
        class="mb-10 control bd-highlight h-50 btn w-full p-5 rounded-3 d-flex align-items-center font-size-16 font-weight-semi-bold overflow-hidden"
        class:bg-dark={!viewHidden}
        class:bg-primary={viewHidden}
        use:click={() => {
          viewHidden = !viewHidden;
        }}
      >
        <span class="ml-20"
          >{lookupHidden?.length} Unseeded Result{lookupHidden?.length > 1
            ? "s"
            : ""} (Unavailable)</span
        >
        <svelte:component
          this={viewHidden ? ChevronUp : ChevronDown}
          class="ml-auto mr-10"
          size="2.2rem"
        />
      </button>
