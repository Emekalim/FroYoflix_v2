<script>
  ...
  $: if (staticMedia?.source === 'TMDB' && staticMedia?.format === 'TV' && !staticMedia?._detailsFetched) {
  ...
  $: if (staticMedia?.source === 'TMDB' && !staticMedia?.trailer?.id && !staticMedia?._trailerFetched) {
    (async () => {
  ...
  $: watched = media && media?.source !== 'TMDB' && media?.mediaListEntry?.status === 'COMPLETED'
  $: userProgress = media && media?.source !== 'TMDB' && ['CURRENT', 'REPEATING', 'PAUSED', 'DROPPED'].includes(media?.mediaListEntry?.status) && media?.mediaListEntry?.progress
  $: missingIds = staticMedia && []
  $: recommendations = staticMedia && staticMedia?.id && staticMedia?.source !== 'TMDB' && anilistClient.recommendations({ id: staticMedia.id })
  $: searchIDS = staticMedia && staticMedia?.id && (async () => {
    ...
    const searchIDS = [...(staticMedia?.relations?.edges?.filter(({ node }) => node.type === 'ANIME').map(({ node }) => node.id) || []), ...(recommendationsData?.data?.Media?.recommendations?.edges?.map(({ node }) => node.mediaRecommendation?.id) || [])]
  ...
  function handlePlay(id, episode, torrentOnly) {
    ...
    const cachedEpisode = isValidNumber(episode) ? episode : (cachedMedia?.source !== 'TMDB' && cachedMedia?.mediaListEntry?.progress)
        ...
        <div class='col-lg-7 col-12 pb-10'>
          <div bind:this={leftColumn}>
            ...
            {#if episodeList?.length}
              <div class='w-full d-flex d-lg-none flex-row align-items-center pt-20 mt-10 pointer' aria-hidden='true'>
                <hr class='w-full' />
                <div class='position-absolute d-flex align-items-center gap-10' style='left: 50%; transform: translateX(-50%);'>
                  {#if staticMedia?.source === 'TMDB' && staticMedia?.format === 'TV'}
                    <span class='font-size-12 font-weight-semi-bold text-white text-nowrap'>Season:</span>
                  ...
                  {:else}
                    <span class='font-size-18 font-weight-semi-bold px-20 text-white'>Episodes</span>
                  {/if}
                </div>
                <hr class='w-full' />
                <div class='ml-auto pl-20 font-size-12 more text-muted text-nowrap pr-20 pointer' use:click={() => { episodeOrder = !episodeOrder }}>Reverse</div>
              </div>
            {/if}
            <div class='col-lg-5 col-12 d-lg-none flex-column mt-20'>
              <EpisodeList bind:episodeList={episodeList} mobileList={true} media={staticMedia} {episodeOrder} {seasonFilter} bind:userProgress bind:watched episodeCount={getMediaMaxEp(media)} {play} class='h-600' />
            </div>
        ...
        <div class='col-lg-5 col-12 d-none d-lg-flex flex-column pl-lg-20' bind:this={rightColumn}>
          {#if staticMedia?.source === 'TMDB' && staticMedia?.format === 'TV'}
            <div class='d-flex flex-row align-items-center mb-20 gap-15'>
              <label for='season-select' class='font-weight-semi-bold text-white text-nowrap mb-0'>Select Season:</label>
              <select id='season-select' bind:value={seasonFilter} class='season-select'>
                {#each Array.from({length: staticMedia?.seasons || 1}, (_, i) => i + 1) as season}
          ...
          <EpisodeList bind:episodeLoad={episodeLoad} media={staticMedia} {episodeOrder} {seasonFilter} bind:userProgress bind:watched episodeCount={getMediaMaxEp(media)} {play} />
        </div>
      </div>
    {/if}
  </div>
