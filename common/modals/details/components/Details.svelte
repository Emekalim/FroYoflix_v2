<script>
  ...
  const detailsMap = [
    { property: 'season', label: 'Season', icon: CalendarRange, custom: 'property', skipIfTMDB: true },
    ...
    { property: 'english', label: 'English', icon: Type, skipIfTMDB: true },
    { property: 'romaji', label: 'Romaji', icon: Languages, skipIfTMDB: true },
    { property: 'native', label: 'Native', icon: '語', custom: 'icon', skipIfTMDB: true }
  ]
  ...
  async function getProperty (property, media) {
    // Skip AniList-only properties for TMDB sources
    if (media?.source === 'TMDB' && ['episode', 'english', 'romaji', 'native', 'studios', 'season'].includes(property)) {
      return false
    }
    if (property === 'episode') {
...
<div bind:this={scrollDetails} class='card m-0 px-20 pb-0 pt-10 d-flex flex-row overflow-x-scroll text-capitalize align-items-start bg-dark-light'>
  {#each detailsMap.filter(d => !d.skipIfTMDB || media?.source !== 'TMDB') as detail}
    {#await media ? getProperty(detail.property, media) : Promise.reject() then property}
      {#if property}
