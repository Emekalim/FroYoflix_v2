...
<script>
  ...
  async function load () {
    // Handle TMDB TV shows differently - use simpler logic
    if (media?.source === 'TMDB') {
      const { getEpisodeMetadataForMedia } = await import('@/modules/anime/anime.js')
      const tmdbEpisodes = await getEpisodeMetadataForMedia(media)
      const episodeCount = Object.keys(tmdbEpisodes).length || (media?.totalEpisodes || media?.episodes || 0)
      
      // Create episode list from TMDB data, filtered by selected season (only for TV shows)
      const seasonEpisodes = media?.format === 'TV' 
        ? Object.entries(tmdbEpisodes).filter(([_, epData]) => epData?.seasonNumber === seasonFilter)
        : Object.entries(tmdbEpisodes) // For movies, show all episodes
      
      episodeList = seasonEpisodes.map(([_, episodeData]) => ({
        episode: episodeData.episodeNumber || episodeData.absoluteEpisodeNumber,
        image: episodeData?.image || null,
        summary: episodeData?.summary || episodeData?.overview || null,
        rating: null,
        title: episodeData?.title?.en || null,
        length: episodeData?.runtime || null,
        airdate: episodeData?.airDate || null,
        airingAt: episodeData?.airDate ? new Date(episodeData.airDate).getTime() / 1000 : null,
        filler: false,
        dubAiring: null,
  ...
  // Reload episodes when season filter changes (for TMDB TV shows)
  $: if (media?.source === 'TMDB' && seasonFilter) {
    if (!mobileList) episodeLoad = load()
  }
