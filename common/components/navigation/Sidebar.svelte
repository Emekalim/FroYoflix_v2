...
<div
  class="sidebar z-80 d-md-block"
  class:animated={$settings.expandingSidebar}
>
  <div
    class="z--1 pointer-events-none h-full bg-dark position-absolute"
    style="width: var(--sidebar-width)"
  />
  <div
    class="sidebar-overlay z--1 pointer-events-none h-full position-absolute"
    class:animated={$settings.expandingSidebar}
  />
  <div
    class="sidebar-menu h-full d-flex flex-column m-0 pb-5 animate"
    class:br-10={!$settings.expandingSidebar}
  >
    <div
      class="w-50 top-0 flex-shrink-0 pointer-events-none {_status?.match(
        /offline/i,
      )
        ? `h-25`
        : `${VERSION.platform === `darwin` && !fullScreen ? `h-25` : `h-0`}`}"
    ...
    {#if !SUPPORTS.isAndroid}
      ...
      >
        <SidebarLink
          click={goBack}
          icon="moveleft"
          css="p-0 m-0 ml-5 h-auto w-30"
          innerCss="rounded-left-block"
        >
          <MoveLeft
            size={"2.5rem"}
            class="flex-shrink-0 rounded m-0"
            strokeWidth="2.5"
            color={$canGoBack ? "currentColor" : "var(--gray-color-very-dim)"}
          />
        </SidebarLink>
        <SidebarLink
          click={goForward}
          icon="moveright"
          css="p-0 m-0 h-auto w-30"
          innerCss="rounded-right-block"
        >
          <MoveRight
            size={"2.5rem"}
            class="flex-shrink-0 rounded m-0"
      ...
      <img
        src="./icon_filled.png"
        tabindex="-1"
        class="m-5 pointer d-sm-h-none"
        style="width: 6rem; height: 6rem"
        alt="ico"
        use:click={() => page.navigateTo(page.HOME)}
      />
    {/if}
    <SidebarLink
      click={() => page.navigateTo(page.HOME)}
      _page={page.HOME}
      icon="home"
      text="Home"
      css={!SUPPORTS.isAndroid ? `mt-md-h-auto` : ``}
      let:active
    >
      <Home
        size={btnSize}
    ...
    <SidebarLink
      click={() => page.navigateTo(page.SEARCH)}
      _page={page.SEARCH}
      icon="search"
      text="Search"
      let:active
    ...
    {#if $settings.donate && !SUPPORTS.isAndroid}
      <SidebarLink
        ...
        css="mt-md-h-auto d-sm-h-none"
        let:active
        let:hovering
      >
        <Heart
          size={btnSize}
          class="flex-shrink-0 p-5 m-5 rounded fill-1 donate"
          strokeWidth="2.5"
          fill="currentColor"
          style="--fill-button-color: {hovering
            ? `var(--gray-color-very-dim)`
            : `var(--quattuordenary-color)`}"
        />
      </SidebarLink>
    {/if}
      ...
      <SidebarLink
        click={() => {
          toast("Update is downloading...", {
            description:
              "This may take a moment, the update will be ready shortly.",
          });
        }}
        icon="download"
        text="Update Downloading..."
        css="{!$settings.donate && !SUPPORTS.isAndroid
          ? `mt-md-h-auto`
          : ``} d-sm-h-none"
        _modal={modal.UPDATE_PROMPT}
        let:active
        let:hovering
      >
        <CloudDownload
          size={btnSize}
          class="flex-shrink-0 p-5 m-5 rounded fill-1"
          strokeWidth="2.5"
          color="currentColor"
          style="--fill-button-color: {hovering
            ? `var(--gray-color-very-dim)`
    ...
    <SidebarLink
      click={() => {
        modal.toggle(modal.NOTIFICATIONS);
      }}
      icon="bell"
      text="Notifications"
      css={!$settings.donate &&
    ...
    >
      {#if $hasUnreadNotifications && $hasUnreadNotifications > 0}
        <BellDot
          size={btnSize}
          class="flex-shrink-0 p-5 m-5 rounded fill-1 notify"
          strokeWidth="2.5"
          color="currentColor"
          style="--fill-button-color: {hovering
            ? `var(--gray-color-very-dim)`
    ...
    <SidebarLink
      click={() => page.navigateTo(page.SETTINGS)}
      _page={page.SETTINGS}
      icon="settings"
      text="Settings"
      let:active
    ...
    {#if !SUPPORTS.isAndroid}
      <SidebarLink
        click={() => modal.toggle(modal.PROFILE)}
        icon="login"
        text={Helper.getUser() ? "Profiles" : "Login"}
        _modal={modal.PROFILE}
        image={Helper.getUserAvatar()}
      >
...
<style>
  ...
  .sidebar .animate :global(.notify) {
    animation:
      purple_glow 1s ease-in-out infinite alternate,
  ...
  .sidebar-overlay.animated {
    transition:
      ...
      left 0.8s cubic-bezier(0.25, 0.8, 0.25, 1) !important;
  }
  ...
  .br-10 {
    border-right: 0.1rem var(--border-color-sp) solid !important;
  }
</style>
