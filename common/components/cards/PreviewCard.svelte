<script>
  import { formatMap, getKitsuMappings } from "@/modules/anime/anime.js";
  ...
  // Lazy-fetch TMDB trailer for preview card
  $: if (
    media?.source === "TMDB" &&
    !media?.trailer?.id &&
    !media?._trailerFetched
  ) {
    (async () => {
      media._trailerFetched = true;
      const { fetchTMDBVideos } = await import("@/modules/sections.js");
      const trailer = await fetchTMDBVideos(media.tmdbId, media.format);
      if (trailer?.id && media) {
...
<div
  class="position-absolute w-350 h-full absolute-container top-0 bottom-0 m-auto bg-dark-light z-30 rounded pointer fade-change overflow-hidden clip-0-rounded"
  in:fadeIn
  out:fadeOut
  bind:this={element}
  on:scroll={(e) => (e.target.scrollTop = 0)}
>
  <div class="banner position-relative bg-black">
    <div class="ratio-16-9 w-full h-full clip-0">
      ...
      />
      {#await (media.trailer?.id && media) || (media?.source !== "TMDB" && episodesList.getMedia(media.idMal)) then trailer}
        {#if trailer?.trailer?.id || trailer?.data?.trailer?.youtube_id}
          {#await ELECTRON.getYouTube() then youtubeServer}
            <div style="transition: opacity .3s" class:transparent={hide}>
              <SmartImage
                class="position-absolute top-0 left-0 w-full h-full img-cover blur-6"
                images={[
                  `https://i.ytimg.com/vi/${media.trailer.id}/maxresdefault.jpg`,
                  `https://i.ytimg.com/vi/${media.trailer.id}/hqdefault.jpg`,
                ]}
              ...
              <button
                type="button"
                class="position-absolute z-10 top-0 right-0 m-15 btn-square bg-transparent shadow-none border-0 rounded pointer mute"
                style="filter: drop-shadow(0 0 .4rem hsla(var(--black-color-hsl), 1))"
              ...
              <iframe
                ...
                allowfullscreen
                on:load={() => {
                  setTimeout(() => (hide = false), 300).unref?.();
                }}
  ...
  <div class="w-full px-20">
    <div
      class="font-scale-20 font-weight-bold text-truncate d-inline-block w-full text-white"
    ...
    {#if !_variables?.fileEdit}
      <div class="d-flex flex-row position-relative">
        <button
          type="button"
          tabindex="-1"
          class="position-absolute preview-safe-area top-0 left-0 h-50 bg-transparent border-0 shadow-none not-reactive"
          use:click={() => {}}
        />
        <button
          class="btn btn-secondary flex-grow-1 text-dark font-weight-bold shadow-none border-0 d-flex align-items-center justify-content-center z-1"
          use:click={play}
        ...
        {#if Helper.isAniAuth()}
          <button
            class="btn btn-square ml-10 d-flex align-items-center justify-content-center shadow-none border-0 z-1"
            data-toggle="tooltip"
            data-placement="top-right"
            data-target-breakpoint="md"
            data-title={media.isFavourite ? "Unfavourite" : "Favourite"}
    ...
    <div class="text-truncate pb-10">
      <div class="details text-white text-capitalize pt-10 d-flex flex-wrap">
        {#if type || type === 0}
          <span
            class="d-flex badge pl-5 pr-5 d-flex align-items-center justify-content-center font-scale-14"
          >
            {#if Number.isInteger(type) && type >= 0}
              <ThumbsUp
                fill="currentColor"
                class="m-0 p-0 pr-5 {type === 0
                  ? 'text-muted'
                  : 'text-success'}"
                size="1.9rem"
              />
            {:else if Number.isInteger(type) && type < 0}
              <ThumbsDown
                fill="currentColor"
                class="text-danger m-0 p-0 pr-5"
                size="1.9rem"
              />
            {/if}
            <span>
        ...
        {#if maxEp > 1 || (maxEp !== 1 && ["CURRENT", "REPEATING", "PAUSED", "DROPPED"].includes(media.mediaListEntry?.status) && media.mediaListEntry?.progress)}
          <span class="badge pl-5 pr-5 font-scale-14">
            {["CURRENT", "REPEATING", "PAUSED", "DROPPED"].includes(
              ...
              : ""}{maxEp &&
            maxEp !== 0 &&
            !(media.mediaListEntry?.progress > maxEp)
              ? maxEp
              : "?"}
            Episodes
          </span>
        ...
        {/if}
        {#if media.isAdult}
          <span class="badge pl-5 pr-5 font-scale-14"> Rated 18+ </span>
        {/if}
        {#await ((media.season || media.seasonYear || media.status === "NOT_YET_RELEASED") && media) || getKitsuMappings(media.id) then details}
          ...
          {#if season || seasonYear || media.status === "NOT_YET_RELEASED"}
            ...
            {#if !season && !seasonYear && media.status === "NOT_YET_RELEASED"}
              <span class="badge pl-5 pr-5 font-scale-14"> Not Released </span>
            {/if}
          {/if}
        {/await}
        {#if media.averageScore}
          <span class="badge pl-5 pr-5 font-scale-14"
            >{media.averageScore + "%"} Rating</span
          >
          {#if media.stats?.scoreDistribution && !type && type !== 0}
            <span class="badge pl-5 pr-5 font-scale-14"
              >{anilistClient.reviews(media)} Reviews</span
            >
          {/if}
        {/if}
      </div>
    ...
    {#if media.description}
      <div
        class="w-full h-full text-muted description overflow-hidden font-scale-14"
      >
        {media.description
          ?.replace(/<[^>]*>/g, "")
          .replace(/\s+/g, " ")
