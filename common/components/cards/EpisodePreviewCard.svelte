  ...
  <div class='image h-200 w-full position-relative d-flex justify-content-between align-items-end text-white'>
    ...
    {#if data.hash || resolvedHash}
      <div class='pr-5 pt-5 z-10 position-absolute top-0 right-0 text-danger icon-shadow'>
        ...
        <TorrentButton class='btn btn-square shadow-none bg-transparent bd-highlight h-40 w-40 z-1 position-relative' hash={[...(data.hash && data.hash !== resolvedHash ? [data.hash] : []), ...(resolvedHash ? [resolvedHash] : [])]} torrentID={data.uri} search={{ media, episode: data.episode, episodeRange: episodeRange }} size={'3rem'} strokeWidth={'2.3'}/>
      </div>
    {/if}
