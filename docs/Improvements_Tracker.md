...
-   **Current State:**
    ...
    -   ✅ **(2026-06-09)** Changed the default managed download root to `Downloads/Froyo Library` on Electron and enabled `torrentPersist` by default, with startup directory creation in `electron/src/main/app.js` plus renderer-side fallback/reset handling in `common/modules/settings.js`, `common/modules/util.js`, and `common/routes/settings/tabs/ClientTab.svelte`.
	    ...
	    -   ✅ **(2026-06-17)** Hardened torrent startup and hard-refresh restore across `client/core/webtorrent.js` and `common/modules/torrent.js` by registering renderer listeners before replaying cached torrent state, restoring cached torrents sequentially instead of fire-and-forget, forcing an immediate post-restore activity sync, and clearing stale staged/seeding cache entries when startup completion scans prove a torrent is already downloaded.
	    -   ✅ **(2026-06-17)** Routed torrent error events into `common/modals/NotificationsModal.svelte` as persisted generic app notifications, and extended the Notifications tab to render non-media entries cleanly so renderer-side torrent failures are visible after the toast disappears.
